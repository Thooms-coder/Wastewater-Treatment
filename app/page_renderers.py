...
def render_chemistry_dosing_page(ctx):
    ...
    with overview_tab:
        ...
        render_help_tip("Dose intensity is presented in mg/L first because that is the clearest engineering concentration view. `lb/day` remains available as secondary feed-rate context. `hcl_active_mg_per_L` and `ferric_active_mg_per_L` use the plant-reported daily dosage from the Chemical Treatment form (the authoritative values, computed against the total transfer to the surge tank). If a few dose spikes flatten the chart, switch axis scaling to `Focused`.")
